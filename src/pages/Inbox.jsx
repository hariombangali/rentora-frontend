...
export default function Inbox() {
    ...
    useEffect(() => {
        ...
        setFiltered(
            conversations.filter((c) => {
                const name = c.partner?.ownerKYC?.ownerName || "";
        ...
        const fetchMessages = async () => {
            ...
            try {
                ...
                const res = await API.get(
                    `/messages/${selectedConversation.partner._id}?propertyId=${selectedConversation.property._id}`,
                    {
                        withCredentials: true,
                        headers: { Authorization: `Bearer ${token}` },
                    }
    ...
    const headerName = selectedConversation?.partner?.ownerKYC?.ownerName || "User";
                ...
                <div className="overflow-y-auto divide-y divide-gray-100">
                    {loadingConvs ? (
                        ...
                        filtered.map((conv, idx) => {
                            const active =
                                selectedConversation?.partner._id === conv.partner._id &&
                                selectedConversation?.property._id === conv.property._id;
                            const name = conv.partner?.ownerKYC?.ownerName || "Unknown";
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-full text-left p-4 transition relative ${active ? "bg-blue-50/70" : "hover:bg-gray-50"
                ...
                {selectedConversation ? (
                    <>
                        ...
                        >
                            {loadingMsgs ? (
                                [...Array(8)].map((_, i) => (
                                    <div key={i} className={`flex gap-2 items-start ${i % 2 ? "justify-end" : "justify-start"} animate-pulse`}>
                                ...
                                messages.map((msg) => {
                                    ...
                                    return (
                                        <div key={msg._id} className={`flex w-full items-end gap-2 ${mine ? "justify-end" : "justify-start"}`}>
                                            {!mine && (
                                            ...
                                            <div
                                                className={`max-w-[75%] md:max-w-[65%] rounded-2xl px-4 py-2.5 shadow-sm transition ${mine ? "bg-blue-600 text-white rounded-br-md" : "bg-gray-100 text-gray-900 rounded-bl-md"
                                                    }`}
                                            >
